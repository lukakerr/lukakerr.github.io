...
- To prove a judgement we can use inference rules, written as
  $$
  \dfrac{J_1 \ \ \ \ J_2 \ \ \ \ \dot{} \dot{} \dot{} \ \ \ \ J_n}{J}
  $$
...
- For example, the natural numbers $\mathbb{N}$
  $$
  \dfrac{}{0 \ Nat} N_1 \ \ \ \ \dfrac{n \ Nat}{(S \ n) \ Nat} N_2
  $$
...
In general, given a set of rules $R$, we may prove a property $P$ inductively for all judgements that can be inferred with $R$ by showing, for each rule of the form $\frac{J_1 \ \ \ \ J_1 \ \ \ \ \dot{} \dot{} \dot{} \ \ \ \ J_n}{J}$ that if $P$ holds for each of $J_1 . . J_n$, then $P$ holds for $J$
...
- A *substitution*, written $e[x := t]$ is the replacement of all free occurances of $x$ in $e$ with tht term $t$
...
- Operational semantics has two main kinds
  ...
  - Big Step
    ...
    - For example, where $E$ is the set of all closed expressions $\{e \ | \ e \ \bold{ok} \}$, and $V$ is the set of integers $\mathbb{Z}$
      $$
      \begin{array}{cc}
      \dfrac{}{(Num \ n) \Downarrow n} & \dfrac{e_1 \Downarrow v_1 \ \ \ \ \ e_2[x := (Num \ v_1)] \Downarrow v_2}{(Let \ e_1 (x. e_2)) \Downarrow v_2} \\ \\
      \dfrac{e_1 \Downarrow v_1 \ \ \ \ \ e_2 \Downarrow v_2}{(Plus \ e_1 \ e_2) \Downarrow (v_1 + v_2)} & \dfrac{e_1 \Downarrow v_1 \ \ \ \ \ e_2 \Downarrow v_2}{(Times \ e_1 \Downarrow e_2) \Downarrow (v_1 \times v_2)}
      \end{array}
      $$
...
- Small and big step semantics can be proven to be equivalent, that is if there exists a trace $e \mapsto \dot{} \dot{} \dot{} \mapsto (Num \ n)$, then $e \Downarrow n$, and vice versa
...
- **Most common:** where upcases do not have dynamic behaviour
...
- $[[e]]\_\mathcal{W}$ denotes work cost and $[[e]]\_\mathcal{D}$ denotes depth cost for an expression $e$
  ...
  - For sequential operations the two measiers coincide. For example:
