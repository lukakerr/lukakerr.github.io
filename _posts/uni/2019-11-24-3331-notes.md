...
When a connection begins, the sending rate is increased exponentially until the first loss is experienced. Initially $cwnd = 1$ MSS. Every $RTT$, $cwnd$ is doubled. The initial sending rate is slow, but increases rapidly.
...
- Sender
  - $cwnd$ is initialised to a small constant
  - $ssthresh$ is initialised to a large constant

#### Events

- ACK (new data)
  - If $cwnd < ssthresh$ then $cwnd = cwnd + 1$ (slow start phase)
  - Else $cwnd = cwnd + 1 / cwnd$ (congestion avoidance phase)
- Duplicate ACK
  - $dupACKcount = dupACKcount + 1$
  - If $dupACKcount = 3$ then $ssthresh = cwnd / 2$ and $cwnd = cwnd / 2$ (fast retransmit)
- Timeout
  - $ssthresh = cwnd / 2$
  - $cwnd = 1$
...
- TCP Tahoe
  - $cwnd = 1$ on triple duplicate ACK and timeout
- TCP Reno
  - $cwnd = 1$ on timeout
