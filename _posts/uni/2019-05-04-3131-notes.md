...
| Type 3  | Regular grammar                 | $A \to w \ \| \ B w$       | Finite state automation |
...
- If $A \to pBq$ is a production, then Follow($B$) includes First($q$) - $\epsilon$
- If $A \to pB$ is a production, then Follow($B$) includes Follow($A$)
- If $A \to pBq$ is a production and First($q$) contains $\epsilon$, then Follow($B$)
includes { First($q$) - $\epsilon$ } $\cup$ Follow($A$)
...
- The select set for a production of the form $A \to \alpha$ is:
  - If $\epsilon \in$ First($\alpha$) then Select($A \to \alpha$) = (First($\alpha$) - { $\epsilon$ }) $\cup$ Follow($A$)
  - Otherwise Select($A \to \alpha$) = First($\alpha$)
...
A grammar is $LL(1)$ if for every nonterminal of the form $A \to \alpha_1 \ \| \ \dots \ \| \ \alpha_n$, the select sets are pairwise disjoint, i.e. Select($A \to \alpha_i$) $\cap$ Select($A \to \alpha_j$) = $\emptyset$ for all $i$ and $j$ such that $i \ne j$.
...
$S \to E \\ E \to E \ / \ E \\ E \to \textbf{num} \\ E \to \textbf{num} \cdot \textbf{num}$
